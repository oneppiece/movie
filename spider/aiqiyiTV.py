...
class aiqiyi(threading.Thread):
    ...
    def get_html_reg(self):
        for u in self.read_list:  # 每个网址
            ...
            try:
                ...
                print
                "第%d个视频 当前网址:%s" % (len(self.write_temp_list_temp), u)
            except Exception, e:
                print
                "打不开！！！", e.message
                continue
    ...
    def write_db_temp(self):
        conn = None
        conn = pymysql.connect(host="122.14.196.95", port=3306, user="root", password="Tenry51885188", db="spider",
                               charset='utf8')
        ...
        for l in self.write_temp_list_temp:  # 一条数据
            ...
            sql = "insert into ku_aiqiyi_temp (v_vid,v_name,v_pu_img_url,rand,create_time) values ('%s','%s','%s',%d,'%s')" % (
                l[0], l[1], l[2], l[3], dt)
            print
            str(l[0]).decode("utf-8"), l[1].encode("utf-8"), l[1].decode("utf-8"), dt
            print
            sql
            cursor.execute(sql)
            print
    ...
    def __parse_video__(self, json_result, v_name, url_temp, pu_img_url, rand):
        ...
        for data in data_list:
            print
            "parse_v_json...name-->%s total_page-->%d current_page-->%d url_temp-->%s" % (
                ...
                for i in range(int(p[0]) + 1):
                    if i <= 1:
                        ...
                        if json_result.get("code") != "A00000":  # 获取不到
                            continue
                        print
                        "parse_loop...total_page-->%d current_page-->%d" % (int(p[0]), i)
                        # time.sleep(1)
                        # print "sleep 1 ..."
